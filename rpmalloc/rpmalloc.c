...
#define STATIC_ASSERT(cond, message) char _static_assert_##__FILE__[(cond) ? 1 : -1]
...
_memory_global_cache_insert(span_t* first_span, size_t list_size, size_t page_count) {
	...
	while (1) {
		...
		if (global_span_ptr != SPAN_LIST_LOCK_TOKEN) {
			...
			/* Swap in the first span pointer */
...
#endif
	{
		...
		size_t ispan;
		for (ispan = 0; ispan < list_size; ++ispan) {
			span_t* next_span;
			assert(first_span);
			next_span = first_span->next_span;
...
_memory_global_cache_extract(size_t page_count) {
	...
	atomic_thread_fence_acquire();
	while ((global_span_ptr = atomic_load_ptr(cache))) {
		if ((global_span_ptr != SPAN_LIST_LOCK_TOKEN) &&
			...
			atomic_store_ptr(cache,
			                 global_span_count ?
			                 ((void*)((uintptr_t)span->prev_span | global_span_count)) :
			                 0);
...
#if MAX_SPAN_CACHE_DIVISOR > 0
	{
		...
		while (1) {
			...
			if (global_span_ptr != SPAN_LIST_LOCK_TOKEN) {
				...
				global_list_size += list_size;
				/* swap in the new global span pointer */
				if (atomic_cas_ptr(cache, (void*)((uintptr_t)span_list | global_list_size), global_span_ptr))
					return;
			}
			else {
				thread_yield();
...
_memory_global_cache_large_extract(size_t span_count) {
	...
	while (global_span_ptr) {
		if ((global_span_ptr != SPAN_LIST_LOCK_TOKEN) &&
			...
			/* store the new global span pointer */
			atomic_store_ptr(cache, global_list_size ?
			                            ((void*)((uintptr_t)span->prev_span | global_list_size)) :
			                            0);
			atomic_thread_fence_release();
			break;
		}
...
_memory_allocate_from_heap(heap_t* heap, size_t size) {
	size_t span_class_idx;
	span_t* span;
	size_t class_idx;
	span_block_t* active_block;
	...
	class_idx = _memory_size_class[(size <= SMALL_SIZE_LIMIT) ?
	...
	active_block = heap->active_block + class_idx;
	size_class = _memory_size_class + class_idx;
...
use_active:
	...
	span_class_idx = _span_class_from_page_count(size_class->page_count);
	...
	/* If we only have one block we will grab it, otherwise set span as new span to use for next allocation */
...
_memory_deallocate_to_heap(heap_t* heap, span_t* span, void* p) {
	...
	++block_data->free_count;
	{
		...
		uint32_t* block = p;
		count_t block_offset;
		count_t block_idx;
		*block = block_data->free_list;
		block_offset = (count_t)pointer_diff(block, span) - SPAN_HEADER_SIZE;
		block_idx = block_offset / (count_t)size_class->size;
...
_memory_deallocate_deferred(heap_t* heap, size_t size_class) {
	...
	atomic_thread_fence_acquire();
	p = atomic_load_ptr(&heap->defer_deallocate);
	if (!p)
		return 0;
	...
	if (p) {
		...
		if (heap_id) {
			if (span->size_class < SIZE_CLASS_COUNT) {
				...
				size_t current_spans;
				if (total_size % SPAN_MAX_SIZE)
					++num_spans;
				current_spans = (span->size_class - SIZE_CLASS_COUNT) + 1;
			...
			current_pages = (size_t)span->next_span;
...
_memory_usable_size(void* p) {
	...
	if (heap_id) {
		...
		/* Large block, return current_span * max_size - header_size */
		return ((span->size_class - SIZE_CLASS_COUNT + 1) * (size_t)SPAN_MAX_SIZE) - SPAN_HEADER_SIZE;
	}
...
_memory_adjust_size_class(size_t iclass) {
	...
	_memory_size_class[iclass].block_count = (uint16_t)(((page_count * PAGE_SIZE) - SPAN_HEADER_SIZE) / block_size);
...
#else
#if ARCH_64BIT
	atomic_store64(&_memory_addr, 0x1000000000UL);
#else
	atomic_store64(&_memory_addr, 0x1000000UL);
#endif
#endif
	...
	for (list_idx = 0; list_idx < HEAP_ARRAY_SIZE; ++list_idx) {
		...
		while (heap) {
			...
			for (iclass = 0; iclass < SPAN_CLASS_COUNT; ++iclass) {
				...
				unsigned int ispan;
				for (ispan = 0; ispan < span_count; ++ispan) {
					span_t* next_span = span->next_span;
...
rpmalloc_thread_statistics(rpmalloc_thread_statistics_t* stats) {
	...
	for (isize = 0; isize < SIZE_CLASS_COUNT; ++isize) {
		span_t* cache;
		...
		for (cache = heap->size_cache[isize]; cache; cache = cache->next_span) {
			stats->sizecache = cache->data.block.free_count * _memory_size_class[cache->size_class].size;
		}
	}

	for (isize = 0; isize < SPAN_CLASS_COUNT; ++isize) {
...
rpmalloc_global_statistics(rpmalloc_global_statistics_t* stats) {
	...
	for (iclass = 0; iclass < SPAN_CLASS_COUNT; ++iclass) {
		void* global_span_ptr;
		while ((global_span_ptr = atomic_load_ptr(&_memory_span_cache[iclass])) == SPAN_LIST_LOCK_TOKEN) {
			thread_yield();
		}
		/* add list bytes */
		stats->cached += /* global span count */ ((uintptr_t)global_span_ptr & ~SPAN_MASK) *
		    (iclass + 1) * SPAN_CLASS_GRANULARITY * PAGE_SIZE;
	}
	for (iclass = 0; iclass < LARGE_CLASS_COUNT; ++iclass) {
		void* global_span_ptr;
		...
		stats->cached_large += /* global span count */ ((uintptr_t)global_span_ptr & ~SPAN_MASK) *
		    (iclass + 1) * SPAN_MAX_PAGE_COUNT * PAGE_SIZE;
	}
}
